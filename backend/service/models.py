...
class Question(Base):
    ...
    # Display format so the test screen renders each IELTS task natively:
    # tfng | ynng (tap buttons), matching (dropdown over the shared list),
    # multi_select (checkboxes, `select_count` picks, graded against
    # `correct_indices` as a set), gap_fill (inline blank). NULL = render by qtype.
    qformat: Mapped[Optional[str]] = mapped_column(String(20))
    correct_indices: Mapped[Optional[list]] = mapped_column(JSON)   # multi_select answer set
    select_count: Mapped[Optional[int]] = mapped_column(Integer)    # how many the student must pick
    # The instruction block printed above the question's task on the paper
    # ("Complete the notes below. Write ONE WORD ONLY from the passage for each
    # answer."). Identical for every question of a task — the test screen groups
    # consecutive questions that share it into one official-style task box.
    task_instructions: Mapped[Optional[str]] = mapped_column(Text)
    ...
    @property
    def marks(self) -> int:
        """Official IELTS question numbers (= marks) this row occupies. A
        multi-select "Choose N letters" task is stored as one row but counts as
        N numbers on the paper (24–26) and is worth N marks."""
        if self.qformat == "multi_select":
            return self.select_count or len(self.correct_indices or []) or 2
        return 1
